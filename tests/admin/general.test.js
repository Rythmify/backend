...
describe('Report rate limiter', () => {
  it('does not enforce rate limit when isDisabled=true (development mode)', async () => {
    // The rate limiter has isDisabled = true globally, so no 429 is ever returned.
    // This test verifies that the 11th request still succeeds (201) rather than being blocked.
    ...
    // Rate limiter is disabled — 11th request also succeeds
    const eleventhResponse = await reloadedRequest(app)
      .post('/reports')
      .send({ resource_type: 'track', resource_id: 'x', reason: 'spam' });

    expect(eleventhResponse.status).toBe(201);
