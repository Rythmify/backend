...
const validateRegister = (req, res, next) => {
  const { email, password, display_name, gender, date_of_birth } = req.body;
  const details = [];
  ...
  if (!password || !isValidPassword(password))
    details.push({ field: 'password', issue: 'Min 8 characters, must include uppercase, lowercase, and a number' });
  ...
  if (!date_of_birth || !isValidDateOfBirth(date_of_birth))
    details.push({ field: 'date_of_birth', issue: 'Must be a valid date and user must be at least 13 years old' });
