...
const getGenreTracks = async ({ genreId, limit = 20, offset = 0, sort = 'newest' }) => {
  // Verify genre exists
  const genre = await genreModel.findGenreDetail(genreId);
  if (!genre) {
    throw new AppError('Genre not found', 404, 'RESOURCE_NOT_FOUND');
...
const getGenreAlbums = async ({ genreId, limit = 12, offset = 0 }) => {
  const genre = await genreModel.findGenreDetail(genreId);
  if (!genre) {
    throw new AppError('Genre not found', 404, 'RESOURCE_NOT_FOUND');
  }
...
const getGenrePlaylists = async ({ genreId, limit = 12, offset = 0 }) => {
  const genre = await genreModel.findGenreDetail(genreId);
  if (!genre) {
    throw new AppError('Genre not found', 404, 'RESOURCE_NOT_FOUND');
  }
...
const getGenreArtists = async ({ genreId, limit = 10, offset = 0, currentUserId = null }) => {
  const genre = await genreModel.findGenreDetail(genreId);
  if (!genre) {
    throw new AppError('Genre not found', 404, 'RESOURCE_NOT_FOUND');
  }
