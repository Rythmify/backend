...
async function resolve(url) {
  ...
  if (first === 'discover') {
    ...
    if (second === 'sets' && third) {
      const mix = await db('mixes').where(bySlugOrId(third, 'mixes')).select('id').first();
      if (!mix) return null;
      return {
        type: 'mix',
        id: mix.id,
        permalink: `https://${ALLOWED_HOSTNAME}/discover/sets/${third}`,
      };
    }
    ...
    if (second === 'stations' && third) {
      ...
      return {
        type: 'station',
        id: station.id,
        permalink: `https://${ALLOWED_HOSTNAME}/discover/stations/${third}`,
      };
    }
    ...
    if (second === 'personalised' && third) {
      ...
      return {
        type: 'made_for_you',
        id: mfy.id,
        permalink: `https://${ALLOWED_HOSTNAME}/discover/personalised/${third}`,
      };
    }
  ...
  if (second === 'tracks' && third) {
    ...
    return {
      type: 'track',
      id: track.id,
      permalink: `https://${ALLOWED_HOSTNAME}/${first}/tracks/${third}`,
  ...
  if (second === 'sets' && third) {
    ...
    return {
      type: 'playlist',
      id: playlist.id,
      permalink: `https://${ALLOWED_HOSTNAME}/${first}/sets/${third}`,
  ...
  if (second === 'album' && third) {
    ...
    return {
      type: 'album',
      id: album.id,
      permalink: `https://${ALLOWED_HOSTNAME}/${first}/album/${third}`,
