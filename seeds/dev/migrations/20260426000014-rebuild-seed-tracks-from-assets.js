...
const { ARTIST_EMAILS, buildAudioManifest } = require('../../seed-audio-manifest');
...
function trackValuesSql() {
  const manifest = buildAudioManifest();
  return manifest.tracks
    .map(
      (track, index) => `(
        ${sqlLiteral(track.ownerEmail)},
        ${sqlLiteral(track.genreName)},
        ${sqlLiteral(track.title)},
        ${sqlLiteral(`Seeded from ${track.filename}.`)},
        ${track.trackNumber},
        ${index + 1}
      )`
    )
...
async function insertManifestTracks(db) {
  await db.runSql(`
    WITH manifest_tracks (
      artist_email, genre_name, title, description, track_number, manifest_position
    ) AS (
      VALUES
      ${trackValuesSql()}
    ),
    inserted_tracks AS (
      INSERT INTO tracks (
        ...
        explicit_content, license_type, created_at
      )
      SELECT
        manifest.title,
        ...
        false,
        'all_rights_reserved',
        NOW() + (manifest.manifest_position * INTERVAL '1 second')
      FROM manifest_tracks manifest
      JOIN users artist
        ON artist.email = manifest.artist_email
       AND artist.role = 'artist'
      JOIN genres genre
        ON genre.name = manifest.genre_name
...
async function reseedDependentData(db) {
...
exports.up = async function (db) {
  ...
  await insertManifestTracks(db);
  await reseedDependentData(db);
};
